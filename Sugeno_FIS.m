...
% plot the MFs (Sugeno: inputs can be plotted with plotmf; outputs need custom)
fig1 = figure(1);
set(fig1, 'Visible', 'on')
...
% Output (Sugeno constants)
nexttile; cla; hold on; grid on;
xlim(constants.F_UNI); ylim([-0.2 0.2]);            % thin band around y=0
yticks([]);                                          % no y-axis
xlabel('f'); title('Sugeno constants');

mf = fis.Outputs(1).MembershipFunctions;             % constants
xs = arrayfun(@(m)m.Parameters, mf);                 % constant values

for k = 1:numel(xs)
    x = xs(k);

    % vertical tick only (no circle marker)
    plot([x x], [-0.15 0.15], 'LineWidth', 2);

    % plain label: show 'f_nb', 'f_ns', ...
    raw = mf(k).Name;                         % e.g., 'f_nb'
    text(x, 0.16, raw, 'Interpreter','none', 'HorizontalAlignment','center');

    % numeric value under the axis
    text(x, -0.16, sprintf('%.3f', x), 'HorizontalAlignment','center');
end
hold off;
...
% Sugfis creation with name p1_mamdani, add input and output variables
function fis = initFis(constants, params)
   
    fis = sugfis('Name', 'p1_sugeno');
    ...
    % Replace trapezoids with CONSTANT MFs (Sugeno zero-order)
    fis = addMF(fis, 'f', 'constant', -39.1667, 'Name', 'f_nb');
    fis = addMF(fis, 'f', 'constant', -20.0000, 'Name', 'f_ns');
    fis = addMF(fis, 'f', 'constant',   0.0000, 'Name', 'f_zo');
    fis = addMF(fis, 'f', 'constant',  20.0000, 'Name', 'f_ps');
    fis = addMF(fis, 'f', 'constant',  39.1667, 'Name', 'f_pb');.
...
for i = 1:numel(T)
    ...
    if mod(i-1, constants.CTRL_STEP) == 0
        ...
        f = runFis(fis, p_err, d_err, constants);  % Extra function call here
